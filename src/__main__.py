...
from . import __version__, diff_values, default_settings, file_direc, IN_EXE
from .minefield import Minefield, Grid
from .utils import get_nbrs, prettify_grid
from .gui.highscores import (
    enchs, get_highscores, settings_keys as hscore_group_keys
    )
...
if '--cli' in sys.argv:
    ...
    from .gui.highscores import HighscoresModel, save_all_highscores
...
class Processor:
    def __init__(self, **settings):
        ...
        for d in diff_values:
            if diff_values[d] == (self.x_size, self.y_size, self.mines):
                self.diff = d
                break
        else:
            self.diff = 'c'
        ...
        self.ui.start()
    def change_difficulty(self, diff, x_size=None, y_size=None, mines=None):
        """Change difficulty, creating a new game with new settings. Arguments
        x_size, y_size, mines are ignored unless diff=='c'."""
        if diff == 'c':
            for d in diff_values:
                if diff_values[d] == (x_size, y_size, mines):
            ...
            self.x_size, self.y_size, self.mines = diff_values[diff]
            self.diff = diff
        else:
    ...
    def finalise_win(self):
        ...
        if self.diff != 'c' and self.game.is_official: #and self.lives==1 etc
            self.hscore = self.game.get_highscore()
            self.ui.handle_highscore(self.hscore)
            # Add completed game to highscores
            # self.current_hscores.append(self.hscore) #moved to gui
    def calculate_probs(self):
        if self.game.state not in ['ready', 'active']:
            return
        if self.game.state == 'active':
            self.game.is_official = False
        ignore_flags = True
        probs = ProbsGrid(self.game.board, ignore_flags, mines=self.mines,
...
class Game:
    ...
    def print_board(self):
        replace = {0:'-', 'U':'#'}
        if hasattr(self, 'per_cell') and self.per_cell == 1:
            for char in ['M', 'F', '!', 'X', 'L']:
                replace[char + '1'] = char
    ...
    def start_game(self, safe_coords=[]):
        for s in ['first_success', 'drag_select', 'per_cell']:
            setattr(self, s, getattr(self.procr, s))
        self.mf.create(self.mines, self.per_cell, safe_coords)
